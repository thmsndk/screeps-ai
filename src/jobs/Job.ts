...
export class Job {
    ...
    public assign(neededWorkers: number, memory: IMemoryJob, role: RoleConstant): number // TODO: memory should be in constructor, will solve later
    {
            ...
            if (this.target) {
                ...
                if (neededWorkers > 0) {
                    for (let index = 0; index < neededWorkers; index++) {
                        ...
                        this.hatchery.queue({ mutation: HARVESTER, target: this.target, priority })
                    }
                }
            }
