// https://github.com/screepers/screeps-stats#enhanced-screeps-stats-collection

const ScreepsStats = function() {
  if (!Memory.___screeps_stats) {
    Memory.___screeps_stats = {}
  }
  this.username =
    _.get(
      _.find(Game.structures, s => true),
      "owner.username",
...
ScreepsStats.prototype.clean = function() {
  const recorded = Object.keys(Memory.___screeps_stats)
  if (recorded.length > this.limit) {
    recorded.sort()
    const limit = recorded.length - this.limit
...
ScreepsStats.prototype.runBuiltinStats = function() {
  ...
  _.forEach(Game.rooms, room => {
    ...
    if (!controller.my) {
      if (!!controller.owner) {
        // Owner is set but is not this user.
...
ScreepsStats.prototype.manageSegments = function(allowedSegments = false) {
  ...
  try {
    const current_data = _.values(Memory.___screeps_stats).reduce(function(acc, val) {
      return _.unique(acc.concat(_.values(val)))
    }, [])
    const unused_segments = _.filter(allowedSegments, function(id) {
      return current_data.indexOf(id) < 0
    })
    var segid = unused_segments[0]
    const json = JSON.stringify(Memory.___screeps_stats[Game.time])

    const maxmemory = 100 * 1024

    const needed_segments = Math.ceil(json.length / maxmemory)
    ...
    for (let i = 0; i < needed_segments; i++) {
      ...
      ids.push(segid)
      const start = i * maxmemory
      const end = start + maxmemory // Will end *one before* this value
