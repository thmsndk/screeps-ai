...
export function summarize_room_internal(room: Room): IRoomSummary | null {
    ...
    const containers = room.find<StructureContainer>(FIND_STRUCTURES, { filter: s => s.structureType == STRUCTURE_CONTAINER });
    ...
    const links = room.find<StructureLink>(FIND_STRUCTURES, { filter: s => s.structureType == STRUCTURE_LINK && s.my });
    ...
    const towers = room.find<StructureTower>(FIND_STRUCTURES, { filter: s => s.structureType == STRUCTURE_TOWER && s.my });
    ...
    const structure_info: StructureInfoDictionary<IStructureInfo> = {};
    ...
    const reduced_resources = _.reduce(ground_resources, (acc: GroundResourcesAmount, res) => { acc[res.resourceType] = _.get(acc, [res.resourceType], 0) + res.amount; return acc; }, {});
...
var summarized_room_timestamp: number = 0
var summarized_rooms: Dictionary<IRoomSummary | null>
export function summarize_rooms() {
    const now = Game.time;

    // First check if we cached it
    if (summarized_room_timestamp == now) {
        return summarized_rooms;
    }

    let retval: Dictionary<IRoomSummary | null> = {};
    ...
    summarized_room_timestamp = now;
    summarized_rooms = retval;
